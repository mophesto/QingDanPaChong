...
class ManifestAutoUpdate:
        ...
        try:
            ...
            with lock:
                if manifest_commit:
                    ...
                    #修改将资源config保存到config.vdf，新增appinfo(保存app信息 DLC(包括独立DLC))，新增config.json(保存depot_id和dlc_id)
    ...
    def run(self, update=False):
        ...
        with Pool(self.pool_num) as pool:
            ...
            for username in account_info_copy:
                if self.update_user_list and username not in account_info_copy:
                    self.log.debug(f'User {username} has skipped the update!')
                    continue
                password, sentry_name = account_info_copy[username]
